...
\begin{homeworkProblem}[Task 1 Database Design]
	...
	\begin{homeworkSection}{Activity 1: First Normal Form}{
		...
		\hspace{3.5cm} \emph{ItemOrder} entity\\
		...
		To complete the First Normal Form, a relationship needs to be created between the entities.
		 ...
		 I created the relationship by repeating the \emph{Order ID} in the \emph{ItemOrder} entity as seen below.\\
		The \emph{Order} entity remains the same, and I have made \emph{Order ID } the primary key as it can be used to uniquely identify each record.\\
		In the \emph{ItemOrder} entity, no one attribute can be used to uniquely identify a single record. For this reason, I have created a concatenated key using the attributes \emph{Equipment} and \emph{Order ID} as a foreign key. The concatenated key can now be used to uniquely identify each record.
		 \vspace{10px}
	...
	\begin{homeworkSection}{Activity 2: Second Normal Form}
		...
		From my testing, I found that the attribute \emph{Unit Price} is not functionally dependant as it is only dependent on \textit{Equipment}, but not\textit{ Order ID}.\\ I moved \emph{Unit Price} into a new entity called \textit{Item} and then I created an relationship between the \emph{Item} and \emph{ItemOrder} entities by repeating the \emph{Equipment} attribute in the \emph{Item} entity. \emph{Equipment} bceomes the primary key.
		
		\vspace{10px}
		
		\emph{Item entity} \hspace{7cm} \emph{Item entity(with relationship)}
		
		\includegraphics[scale = 0.8]{Task1/ItemO1.png} \includegraphics[scale = 0.2]{Task1/arrow.jpg} \includegraphics[scale = 0.8]{Task1/Item02.png}
		
		
		\subsubsection{All three entities in Second Normal Form}
