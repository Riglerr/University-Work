...
int main()
{
  ...
  while (1) {
    ...
    {
        //If number then add it to str_temp
        if(line[i] >='0' && line[i] <= '9'){
        str_temp+=line[i];
        }
        // if separator char (SPACE)
        else if(line[i] == ' ')
        {
          // if str_temp has a value in it,
          //  then push and clear the string.
            if(!str_temp.empty()){
                push(atoi(str_temp.c_str()));
                str_temp.clear();}
                // else go to next char in input string.
            else
                continue;
        ...
        // if operator
          ...
          // pop two value from stack.
        int a = pop();
        int b = pop();

          // Do operation depending on operator
...
// Only value left in stack is the answer.
sprintf(line,"%d",pop());
